...
public class Seamap {
	...
	public void putShipOnSeamap(Ship ship, Coords fore, Direction direction)
		...
		/**
		 * Falls das Schiff nach unten ausgerichtet ist, wird x nicht erhöht und
		 * nur y verändert, weshalb der X-Startwert aus der Variablen fore
		 * genommen werden soll Was das bringt? Es erspart eine weitere
		 * for-Schleife. Keine Angst, es gibt für diese Methode diverse Tests^^
		 * 
		 * **/
		try {
			iterateOverShipPartsAndSetNewValues(fore, direction,
					ship.getShipParts());
			GameManager.getLogger().debug("Koordinaten des Schiffes: X=" + fore.getX() + " Y=" + fore.getY());
		} catch (IllegalShipPlacementException e) {
			try {
				removeShipParts(fore, direction,
						e.getFailureAtShipSegmentNumber());
	...
	private void removeShipParts(Coords fore, Direction direction, int part)
			throws IllegalShipPlacementException {
	...
	private void iterateOverShipPartsAndSetNewValues(Coords fore,
			Direction direction, ShipPart[] newValues)
			throws IllegalShipPlacementException {

		for (int startwert = (fore.getX() * direction.getModX())
				+ (fore.getY() * direction.getModY()), segment = 0; segment < newValues.length; segment++, startwert++) {
			Coords coords = new Coords(
					((direction.getModX() * startwert) + (fore.getX() * direction
							.getModY())),
					((direction.getModX() * fore.getY()) + (startwert * direction
							.getModY())));

			try {
				if (legitShipPlacement(coords, newValues[segment])) {

					shipParts[(direction.getModX() * startwert)
							+ (fore.getX() * direction.getModY())][(direction
							.getModX() * fore.getY())
							+ (startwert * direction.getModY())] = newValues[segment];

				} else {
					GameManager.getLogger().error(
							Messages.getString("Seamap.LogTextIllegalPlacementNearAnotherShip")); //$NON-NLS-1$
				...
				GameManager.getLogger().error(
						Messages.getString("Seamap.LogTextIllegalPlacementOutsideMap")); //$NON-NLS-1$
				throw new IllegalShipPlacementException(
	...
	public ShipPart getShipPart(Coords coords) {
		if ((coords.getX() >= 0) && (coords.getY() >= 0)
				&& (coords.getX() < size) && (coords.getY() < size)) {
	...
	private boolean theseCoordsAndSurroundingCoordsAreFree(Coords coords,
			ShipPart shipPartToPlace) {

		for (int x = -1; x < 2; x++) {
			for (int y = -1; y < 2; y++) {
				ShipPart excistingShipPart = null;
				try {
					excistingShipPart = shipParts[coords.getX() + x][coords
							.getY() + y];
				...
				if (excistingShipPart != null) {
					...
					if (!shipPartBelongsToGivenShip(shipPartToPlace,
							excistingShipPart.getShip())) {
						return false;
					}
				}
			}
	...
	private boolean shipPartBelongsToGivenShip(ShipPart givenShipPart,
			Ship givenShip) {
